...
export default async function CategoryPage({ params }: Props) {
  ...
  const structuredData = {
    '@context': 'https://schema.org', '@type': 'CollectionPage', name: `${category.shortLabel} في نقادة`, url: pageUrl, description: category.description,
    mainEntity: { '@type': 'ItemList', numberOfItems: scoped.length, itemListElement: scoped.slice(0, 20).map((item, index) => ({ '@type': 'ListItem', position: index + 1, name: item.name, url: `${siteConfig.url}/listing/${encodeURIComponent(item.slug)}` })) },
  };
  return (
    <main id="main-content" className="page-main">
      ...
      <section className="shell page-section"><Suspense fallback={<div className="loading-state">جارٍ تجهيز النتائج…</div>}><DirectoryExplorer businesses={scopedDirectory} categories={categories} localities={localities} initialCategory={category.name} lockedCategory /></Suspense></section>
