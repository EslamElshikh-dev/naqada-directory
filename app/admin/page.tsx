...
export default async function AdminPage() {
  ...
  const [stats, analyticsResult, discoveryResult, ownerResponse, moderatorReport] = await Promise.all([
  ...
  const publishedOwnerCount = ownerStatuses.filter((item) => item.status === 'published').length;
  ...
  const suggestions = buildSuggestions(analytics, discovery, stats.pendingContributions + pendingOwnerCount);
  ...
  return (
    <main id="main-content" className="admin-page admin-page--premium">
      ...
      <div className="shell admin-shell admin-shell--premium">
        ...
        <section id="moderator-performance" className={reportStyles.report} aria-label="أداء المشرفة الذهبية"><header><div><span>فريق الإشراف</span><h2>نشاط آية رفاعي</h2><p>الأرقام من سجل الإجراءات الفعلي خلال آخر ٣٠ يومًا، وتُحدّث عند فتح اللوحة.</p></div><Link href="/moderator">فتح مساحة الإشراف ←</Link></header>{moderatorReport ? <><div className={reportStyles.summary}><article><span>المشرفة</span><strong>{moderatorReport.moderator?.name || 'لم تُفعّل العضوية بعد'}</strong><small>{moderatorReport.moderator?.status === 'active' ? 'حساب نشط' : moderatorReport.moderator?.status || 'بانتظار التفعيل'}</small></article><article><span>إجراءات ٣٠ يومًا</span><strong>{format(moderatorReport.actions30d)}</strong><small>مراجعة ونشر وإدارة أعضاء</small></article><article><span>آخر إجراء</span><strong>{moderatorReport.recent.length ? timeLabel(moderatorReport.recent[0].at) : 'لا يوجد بعد'}</strong><small>من سجل المشرفة</small></article></div><div className={reportStyles.details}><div><h3>نوع الإجراءات</h3>{moderatorReport.actionsByType.length ? <ul>{moderatorReport.actionsByType.map((item) => <li key={item.action}><span>{item.action.replaceAll('_', ' ')}</span><b>{format(item.count)}</b></li>)}</ul> : <p className={reportStyles.empty}>لم تُسجل إجراءات بعد.</p>}</div><div><h3>آخر النشاطات</h3>{moderatorReport.recent.length ? <ul>{moderatorReport.recent.slice(0, 6).map((item, index) => <li key={`${item.at}-${index}`}><span>{item.action.replaceAll('_', ' ')} · {item.kind}</span><small>{timeLabel(item.at)}</small></li>)}</ul> : <p className={reportStyles.empty}>هتظهر هنا أول مراجعة تعملها آية.</p>}</div></div></> : <p className={reportStyles.empty}>تعذّر تحميل سجل المشرفة الآن؛ حدّث الصفحة بعد قليل.</p>}</section>
