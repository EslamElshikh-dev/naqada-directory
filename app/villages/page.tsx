...
export default function VillagesPage() {
  ...
  const structuredData = { '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'قرى ونجوع مركز نقادة', url: `${siteConfig.url}/villages`, mainEntity: { '@type': 'ItemList', numberOfItems: localities.length, itemListElement: localities.map((item, index) => ({ '@type': 'ListItem', position: index + 1, name: item.name, url: `${siteConfig.url}/villages/${encodeURIComponent(item.slug)}` })) } };
  return (
    <main id="main-content" className="page-main">
      <section className="geo-hero"><div className="shell geo-hero__grid"><div><nav className="breadcrumbs"><Link href="/">الرئيسية</Link><span>/</span><span>القرى والنجوع</span></nav><span className="eyebrow">الجغرافيا المحلية</span><h1>قرى ونجوع <em>مركز نقادة</em></h1><p>تصفّح الهيكل الجغرافي الموثق، ثم انتقل إلى الخدمات المنشورة في كل مدينة أو قرية أو نجع أو عزبة.</p><a href="#places" className="button button--light">استكشف المواضع</a></div><aside className="geo-hero__panel"><BrandMark /><strong>مركز نقادة · قنا</strong><p>صفحات مستقلة تحافظ على اسم الموضع ونوعه ونطاقه كما ورد في المصدر.</p><div className="catalog-hero__metrics"><span><b>{localities.length.toLocaleString('ar-EG')}</b><small>موضعًا</small></span><span><b>{kinds.toLocaleString('ar-EG')}</b><small>أنواع جغرافية</small></span><span><b>{businesses.length.toLocaleString('ar-EG')}</b><small>سجلًا خدميًا</small></span></div></aside></div></section>
      <section id="places" className="shell page-section"><div className="section-heading"><div><span className="eyebrow eyebrow--dark">استكشف حسب المكان</span><h2>صفحة مستقلة لكل مدينة وقرية ونجع وعزبة</h2><p>تشمل القائمة المواضع الرسمية والمواضع الإضافية المثبتة من بيانات الأنشطة أو المصادر المحلية، مع توضيح حالة كل سجل داخل صفحته.</p></div></div><LocalityExplorer localities={localities} /></section>
