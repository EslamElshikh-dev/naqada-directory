...
function JobCard({ job }: { job: LocalJob }) {
  ...
  return <article className={`${styles.jobCard} ${job.kind === 'seeker' ? styles.seekerCard : ''}`} id={`job-${job.id}`}>
    <div className={styles.cardTop}><span className={styles.cardSymbol} aria-hidden="true">{job.kind === 'seeker' ? '✦' : '↗'}</span><span className={styles.cardPill}>{job.kind === 'offer' ? (isExternal ? 'من مصدر خارجي' : 'فرصة من أهل البلد') : 'باحث عن شغل'}</span><time dateTime={job.published_at}>{dateLabel(job.published_at)}</time></div>
    <h3>{job.title}</h3>
...
export function JobsBoard({ jobs, offers, seekers, state, available, localities }: { jobs: LocalJob[]; offers: number; seekers: number; state: JobFeedState | null; available: boolean; localities: string[] }) {
  ...
  return <>
    <section id="opportunities" className={`shell ${styles.board}`}>
      ...
      <p className={styles.sourceNote}>نراجع المصادر العامة المتاحة كل ٣٠ دقيقة. ظهور إعلان خارجي يعتمد على نشره في مصدر مفتوح وحداثة تاريخه؛ وروابط السوشيال يقدر أصحاب الإعلانات يضيفوها مع طلبهم. راجع التفاصيل من المصدر قبل التقديم.</p>
    </section>
