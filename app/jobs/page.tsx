...
export const metadata: Metadata = {
  ...
  description: 'فرص عمل في نقادة وقراها، وفرص أخرى بمحافظة قنا مع مكان كل وظيفة ورابط التقديم. أضف وظيفة أو اعرض خبراتك للبحث عن شغل.',
...
export default async function JobsPage() {
  ...
  const localOffers = offers.filter((job) => localitiesList.includes(job.locality));
  const regionalOffers = offers.filter((job) => !localitiesList.includes(job.locality));
  const structuredData = {
    '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'وظائف نقادة وقراها', url: absoluteUrl('/jobs'),
    description: 'فرص عمل في مركز نقادة وقراه، وفرص أخرى في محافظة قنا مع توضيح مكان العمل.', inLanguage: 'ar-EG',
  ...
  return <main id="main-content" className={styles.page}>
      ...
      <div className={`shell ${styles.heroShell}`}>
        ...
        <div className={styles.heroGrid}>
          <div className={styles.heroCopy}>
            ...
            <p>شوف فرص نقادة وقراها، ولو حابب توسّع الدايرة هتلاقي فرص محافظة قنا بمكانها الحقيقي ورابط إعلانها. وعندك شغل للناس؟ انشر التفاصيل هنا.</p>
    ...
    <JobsBoard jobs={jobs} offers={localOffers.length} regionalOffers={regionalOffers.length} seekers={seekers.length} state={state} available={available} localities={localitiesList} />
