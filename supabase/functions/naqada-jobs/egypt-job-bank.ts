...
const LIST_URLS = ['https://egjobank.com/jobs/in/qena', 'https://egjobank.com/jobs/in/luxor'];
...
export function readEgyptJobBankPosting(html: string, url: string, now = Date.now()) {
  ...
  const governorate = /الأقصر|الاقصر|luxor/i.test(region) ? 'الأقصر' : /قنا|qena/i.test(region) ? 'قنا' : null;
  if (!governorate || !location || /القناطر/.test(location)) return null;
  ...
  return { kind: 'offer', origin: 'external', status: 'published', title, organization: company,
    locality: governorate === 'الأقصر' ? canonicalLuxorLocality(location) : location === 'نقادة' ? 'مدينة نقادة' : location, governorate, field: `وظائف محافظة ${governorate}`,
...
export async function scanEgyptJobBank() {
  const scans = await Promise.all(LIST_URLS.map(async (listing) => {
    try {
