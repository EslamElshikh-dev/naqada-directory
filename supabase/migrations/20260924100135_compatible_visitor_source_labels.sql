-- Preserve readable source labels on the currently deployed admin dashboard.
-- Each visitor still belongs to one source and device in the reporting window.
        ...
        from (
          select distinct on (visitor_id) visitor_id,
            case
              ...
              when referrer_host ~ '(^|[.])google[.]' then 'www.google.com'
              when referrer_host ~ '(^|[.])bing[.]' then 'bing.com'
              when referrer_host ~ '(^|[.])facebook[.]' or referrer_host ~ '(^|[.])fb[.]' then 'www.facebook.com'
              when referrer_host ~ '(^|[.])whatsapp[.]' then 'web.whatsapp.com'
              else referrer_host
            end as source
